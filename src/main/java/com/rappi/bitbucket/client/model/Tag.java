/*
 * Bitbucket API
 * Code against the Bitbucket API to automate simple tasks, embed Bitbucket data into your own site, build mobile or desktop apps, or even add custom UI add-ons into Bitbucket itself using the Connect framework.
 *
 * OpenAPI spec version: 2.0
 * Contact: support@bitbucket.org
 *
 * NOTE: This class is auto generated by the swagger code generator program.
 * https://github.com/swagger-api/swagger-codegen.git
 * Do not edit the class manually.
 */

package com.rappi.bitbucket.client.model;

import java.util.Objects;
import java.util.Arrays;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.annotation.JsonCreator;
import com.fasterxml.jackson.annotation.JsonValue;
import com.rappi.bitbucket.client.model.Author;
import com.rappi.bitbucket.client.model.Commit;
import com.rappi.bitbucket.client.model.Ref;
import com.rappi.bitbucket.client.model.RefLinks;
import io.swagger.v3.oas.annotations.media.Schema;
import org.threeten.bp.OffsetDateTime;
/**
 * Tag
 */

@javax.annotation.Generated(value = "io.swagger.codegen.v3.generators.java.JavaClientCodegen", date = "2020-11-12T14:05:09.453-06:00[America/Mexico_City]")
public class Tag extends Ref {
  @JsonProperty("message")
  private String message = null;

  @JsonProperty("date")
  private OffsetDateTime date = null;

  @JsonProperty("tagger")
  private Author tagger = null;

  public Tag message(String message) {
    this.message = message;
    return this;
  }

   /**
   * The message associated with the tag, if available.
   * @return message
  **/
  @Schema(description = "The message associated with the tag, if available.")
  public String getMessage() {
    return message;
  }

  public void setMessage(String message) {
    this.message = message;
  }

  public Tag date(OffsetDateTime date) {
    this.date = date;
    return this;
  }

   /**
   * The date that the tag was created, if available
   * @return date
  **/
  @Schema(description = "The date that the tag was created, if available")
  public OffsetDateTime getDate() {
    return date;
  }

  public void setDate(OffsetDateTime date) {
    this.date = date;
  }

  public Tag tagger(Author tagger) {
    this.tagger = tagger;
    return this;
  }

   /**
   * Get tagger
   * @return tagger
  **/
  @Schema(description = "")
  public Author getTagger() {
    return tagger;
  }

  public void setTagger(Author tagger) {
    this.tagger = tagger;
  }


  @Override
  public boolean equals(java.lang.Object o) {
    if (this == o) {
      return true;
    }
    if (o == null || getClass() != o.getClass()) {
      return false;
    }
    Tag tag = (Tag) o;
    return Objects.equals(this.message, tag.message) &&
        Objects.equals(this.date, tag.date) &&
        Objects.equals(this.tagger, tag.tagger) &&
        super.equals(o);
  }

  @Override
  public int hashCode() {
    return Objects.hash(message, date, tagger, super.hashCode());
  }


  @Override
  public String toString() {
    StringBuilder sb = new StringBuilder();
    sb.append("class Tag {\n");
    sb.append("    ").append(toIndentedString(super.toString())).append("\n");
    sb.append("    message: ").append(toIndentedString(message)).append("\n");
    sb.append("    date: ").append(toIndentedString(date)).append("\n");
    sb.append("    tagger: ").append(toIndentedString(tagger)).append("\n");
    sb.append("}");
    return sb.toString();
  }

  /**
   * Convert the given object to string with each line indented by 4 spaces
   * (except the first line).
   */
  private String toIndentedString(java.lang.Object o) {
    if (o == null) {
      return "null";
    }
    return o.toString().replace("\n", "\n    ");
  }

}
